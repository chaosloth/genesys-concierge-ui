	...
	function ( jqm, ixn ) { 
		...
		var createTask =  function(person) {
			...
			var data = {
				...
				'LicenseNumber' : person.licenseno,
				'PhoneNumber' : person.licenseno,
				'TicketNumber' : ticket,
				'url1' : url,
				'viewname' : 'SAAQ'
			};
		...
		$('.btnEnSubmit').on('click', function(){
			...
			person.licenseno = $("#enNewInteraction #licenseno").val();
			person.task = task;
			person.lang = lang;
			createTask(person);
		...
		$('.btnFrSubmit').on('click', function(){
			...
			person.licenseno = $("#frNewInteraction #licenseno").val();
			person.task = task;
			person.lang = lang;
			createTask(person);
		...
		$('.btnComplete').on('click', function(){
			...
			$("#frNewInteraction #licenseno").val('');
			$("#frNewInteraction #ticket").val('');
			
			$("#enNewInteraction #firstname").val('');
			$("#enNewInteraction #licenseno").val('');
